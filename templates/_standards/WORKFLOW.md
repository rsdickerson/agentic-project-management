...
**Worker polling stop** - After reviewing a Worker's report, when that Worker has no `Ready` Tasks and receives no dispatch in the same review cycle, the Manager runs `.apm/scripts/stop-task-polling.sh` so the Worker stops polling and the session can end. When work becomes Ready later, dispatch restarts the Worker via `{COMMAND_SLUG:work}` or `{COMMAND_SLUG:task}` as needed.
