...
Workers read their Task Bus when the User runs `{COMMAND_SLUG:task}` in the Worker's chat, or automatically via work polling after assignment completion and on operator interaction when idle (see `{GUIDE_PATH:task-execution}` §3.7). The Manager reads Report Buses when the User runs `{COMMAND_SLUG:review}`. Both commands accept optional agent identifier arguments for targeted delivery.
...
2. User runs the indicated command(s) in the Worker's context, or sends a message to an idle initialized Worker (which triggers automatic Task Bus re-check via work polling).
3. Worker executes the Task, writes a Task Log, writes a Task Report to the Report Bus, and directs the User to deliver the report - including the agent identifier for targeted retrieval. After completion, the Worker automatically checks the Task Bus for additional assignments (same-turn exhaustion when work is queued) or enters idle state (re-checking on next operator interaction).
...
**Work polling** - After Task Completion, the Worker performs Work Queue Check (see `{GUIDE_PATH:task-execution}` §3.7): automatically picks up queued assignments in the same conversation turn (same-turn exhaustion), announces idle readiness when the queue is empty, re-checks the Task Bus on subsequent operator interactions, and stops when handoff, explicit operator direction, or a best-effort ~75% session context threshold is reached. `{COMMAND_SLUG:task}` remains available as init trigger and manual fallback but is not required between back-to-back queued assignments for initialized Workers.
